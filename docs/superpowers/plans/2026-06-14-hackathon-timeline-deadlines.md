...
**CRITICAL:** `Events.ts` has `versions: { drafts: true }` (line 76), so the `events` collection has a `_events_v` version twin. Every prior migration adding columns to `events` mirrors them onto `_events_v` as `version_<name>` (see `20260612c_event_timezone.ts`). The deadline columns MUST be mirrored too, or wiring the fields into the collection (Task 2) hits schema drift on the version table. Unlike the timezone migration, NO backfill is needed — deadlines are legitimately null.
