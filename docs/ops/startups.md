...
- **Staging index gate:** while the verified (approved) row count is **< 3000**,
  ...
  `generateMetadata` flips from the live count. The sitemap builder strips any
  leaked `/investigations/startups` URL unless the gate listed paths. The
  code auto-flips robots + sitemap at **≥ 3000**. That flip is a crawl switch
  only — **Writing Bot + Ops Pass are still required before claiming a public
  Investigation Pass.** Do not invent a Pass flag. Promo (LinkedIn /
  newsletter / Hub push) also waits for ≥3000.
