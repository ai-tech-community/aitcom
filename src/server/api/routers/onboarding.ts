...
export const onboardingRouter = createTRPCRouter({
  ...
  syncAutoDetected: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    const steps = getStepsForIntent(profile.onboardingIntent);
    const autoDetectSlugs = steps
      .filter((s) => s.autoDetect)
      .map((s) => s.slug);
    ...
    const [threadAct] = await ctx.db
      ...
      .limit(1);
    if (threadAct) {
      if (autoDetectSlugs.includes("forum_intro"))
        autoCompleted.push("forum_intro");
      if (autoDetectSlugs.includes("create_thread"))
        autoCompleted.push("create_thread");
    }
  ...
  getSuggestions: protectedProcedure.query(async ({ ctx }) => {
    ...
    const matched = allMembers
      .filter((m) => m.skills.some((s) => tagSet.has(s.toLowerCase())))
      .slice(0, 5);
