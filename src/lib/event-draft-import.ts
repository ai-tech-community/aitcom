...
export const discoveredEventImportSchema = z.object({
  ...
  reviewStatus: z.enum(EVENT_REVIEW_STATUS_OPTIONS).optional().default("discovered"),
...
export type DiscoveredEventImportInput = z.input<typeof discoveredEventImportSchema>;
export type DiscoveredEventImport = z.infer<typeof discoveredEventImportSchema>;
export type EventDraftImportPayload = ReturnType<typeof buildEventDraftImportPayload>;
 ...
 */
export function buildEventDraftImportPayload(input: DiscoveredEventImportInput) {
...
export function buildEventDraftImportBatch(inputs: DiscoveredEventImportInput[]) {
  return discoveredEventBatchSchema.parse(inputs).map(buildEventDraftImportPayload);
}
