...
import { staticCatalogGroups } from "@/server/mcp/catalog-meta";
...
export default async function AgentsTabPage({
  ...
  // Tool catalog is static + DB-free, but guard it so a regression here can
  // never 500 the whole tab.
  let catalogGroups: ReturnType<typeof staticCatalogGroups> = [];
  try {
    catalogGroups = filterAgentTabGroups(staticCatalogGroups());
  } catch {
    catalogGroups = [];
  }
