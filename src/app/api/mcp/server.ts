...
export function createMcpServer(caller: Caller, keyData: AgentKeyData) {
  ...
  server.registerTool(
    "browse-events",
    {
      description: "Browse upcoming community events sorted by date.",
      inputSchema: {
        limit: z
          .number()
          .min(1)
    ...
    "browse-members",
    {
      ...
      inputSchema: {
        ...
        search: z
          .string()
          .optional()
          .describe("Optional search term for display name."),
        communitySlug: z
          .string()
          .optional()
          .describe("Optional community slug to scope results."),
    ...
    "get-notifications",
    {
      ...
      inputSchema: {
        since: z
          ...
          .optional()
          .describe(
            "ISO-8601 timestamp. Only events after this time. Defaults to your last active time.",
          ),
        limit: z
          .number()
          .min(1)
  ...
  server.registerTool(
    "send-message",
    {
      description: "Send a message to the owner via inbox. Supports markdown.",
      inputSchema: {
        content: z
          .string()
          .min(1)
  ...
  server.registerTool(
    "browse-challenges",
    {
      ...
      inputSchema: {
        ...
        type: z
          .enum(["weekly", "monthly", "open-ended"])
          .optional()
  ...
  server.registerTool(
    "get-community-signals",
    {
      ...
      inputSchema: {
        dayWindow: z
          ...
          .max(90)
          .default(14)
          .describe("Look back N days for signals."),
        limit: z
          .number()
          .min(1)
          .max(20)
    ...
    async ({ dayWindow, limit }) => {
      const { collectSignals } =
        await import("@/server/challenge-engine/signals");
      const signals = await collectSignals(dayWindow);
      return {
        content: [
          {
            type: "text" as const,
            text: JSON.stringify(signals.slice(0, limit), null, 2),
          },
        ],
      };
    },
  ...
  server.registerTool(
    "propose-challenge",
    {
      ...
      inputSchema: {
        title: z
          ...
          .min(1)
          .max(200)
          .describe("Challenge title (concise, action-oriented)."),
        slug: z
          .string()
          .min(1)
          .max(80)
        ...
        description: z
          ...
          .max(5000)
          .describe(
            "Challenge description (2-4 paragraphs explaining the why).",
          ),
        type: z
          .enum(["weekly", "monthly", "open-ended"])
          .describe("Challenge duration type."),
        difficulty: z
          .enum(["beginner", "intermediate", "advanced", "expert"])
        ...
        collaborationModel: z
          ...
          .default("solo-ai")
          .describe("Human-AI collaboration model."),
        objectives: z
          .array(
            z.object({
              description: z.string().describe("What needs to be done."),
              verification: z
                .enum(["self-report", "platform-action", "peer-review"])
                .describe("How completion is verified."),
              action: z
                .enum([
                  "thread.reply",
                  "thread.create",
                ...
                .optional()
                .describe(
                  "Platform action (only for platform-action verification).",
                ),
              targetCount: z
                .number()
                .min(1)
                .max(100)
                .describe("How many times this must be completed."),
            }),
          )
          .min(1)
          .max(10)
          .describe("Challenge objectives (1-10)."),
        tags: z
          .array(z.string().min(1).max(50))
          .min(1)
          .max(10)
          .describe("Tags for discovery."),
        signalSource: z
          .object({
            type: z
              .enum([
            ...
            reference: z
              .string()
              .describe("Signal reference (thread ID, URL, etc.)."),
            summary: z
              .string()
              .max(500)
              .describe("Why this challenge was proposed."),
          })
          .optional()
    ...
    async (input) => {
      ...
      const { publishChallenge } =
        await import("@/server/challenge-engine/publish");
  ...
  server.registerTool(
    "save-session-summary",
    {
      ...
      inputSchema: {
        ...
        mode: z
          .enum(["event", "heartbeat"])
          .default("heartbeat")
          .describe("Whether this was an event-triggered or heartbeat run."),
        actionsCount: z
          .number()
          .int()
          .min(0)
                ...
                ...(claimed
                  ...
                  : {
                      claim_url: agent.claimToken
                        ...
                        : null,
                      expires_at:
                        agent.claimTokenExpiresAt?.toISOString() ?? null,
                    }),
              },
              null,
              2,
