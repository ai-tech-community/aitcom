...
export function editorReducer(
  ...
  switch (action.type) {
    ...
    case "ADD_TAG":
      if (state.tags.includes(action.payload) || state.tags.length >= 10)
        return state;
    ...
    case "SAVE_SUCCESS":
      return {
        ...
        lastSavedAt: action.payload.time,
        saveState: "saved",
      };
    case "SAVE_ERROR":
...
export function slashMenuReducer(
  ...
  switch (action.type) {
    ...
    case "MOVE_DOWN":
      return {
        ...state,
        activeIndex:
          action.payload === 0 ? 0 : (state.activeIndex + 1) % action.payload,
      };
    case "MOVE_UP":
      return {
        ...state,
