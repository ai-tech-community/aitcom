...
const StreakCard = React.forwardRef<HTMLDivElement, StreakCardProps>(
  (
    ...
    const action = actionLabel ?? t("viewDetails");
    const howTitle = howItWorksTitle ?? t("howTitle");
    const howItems = howItWorksItems ?? [t("how1"), t("how2"), t("how3")];

    return (
      <section
        ref={ref}
        aria-label={t("ariaSummary")}
        className={cn("bg-card border-border rounded-xl border p-6", className)}
        {...props}
      >
        <header className="mb-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <Flame className="text-success h-6 w-6" aria-hidden="true" />
            <h3 className="text-2xl leading-none font-semibold">{heading}</h3>
          </div>
          {onActionClick && (
            <Button
              variant="link"
        ...
        <p className="mb-4 font-mono text-5xl leading-none font-semibold tracking-tight">
          {currentStreak}
        ...
        <div
          className="border-border mt-4 grid grid-cols-2 gap-4 border-t border-dashed pt-4"
          aria-label={t("ariaStats")}
        >
          <div>
            <p className="text-muted-foreground text-sm">{t("longest")}</p>
            <p className="font-mono text-3xl leading-tight font-semibold">
              {longestStreak}
              <span className="ml-1 text-2xl font-medium">{t("days")}</span>
            </p>
          </div>
        ...
        {showHowItWorks && (
          <div className="border-border mt-4 border-t pt-4">
            ...
            >
              <span className="text-lg font-semibold">{howTitle}</span>
              <ChevronDown
                className={cn(
