...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  // ── forum_threads: add missing columns ──────────────────────────────
  await db.execute(sql`
    ALTER TABLE "forum_threads"
      ADD COLUMN IF NOT EXISTS "community_id" varchar,
  ...
  await db.execute(sql`
    CREATE INDEX IF NOT EXISTS "forum_threads_community_id_idx"
      ON "forum_threads"("community_id");
  `);

  // ── forum_replies: add missing columns ──────────────────────────────
  await db.execute(sql`
    DO $$ BEGIN
  ...
  await db.execute(sql`
    CREATE INDEX IF NOT EXISTS "forum_replies_community_id_idx"
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  // Remove added columns from forum_replies
  await db.execute(sql`
    ALTER TABLE "forum_replies"
  ...
  // Remove added columns from forum_threads
  await db.execute(sql`
    ALTER TABLE "forum_threads"
      DROP COLUMN IF EXISTS "edited_at",
      DROP COLUMN IF EXISTS "is_edited",
